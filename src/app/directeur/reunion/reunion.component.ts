...
export class ReunionComponent implements OnInit {
  ...
  listeReunionTeamSuperviseur: ReunionRestSup[];
  getSuperviseur = false;
  listeReunionTeamInspecteur: ReunionRestSup[];
  getInspecteur = false;
  listeReunionTeamResponsable: ReunionRestSup[];
  getResponsable = false;
  listeReunionTeamDiradjoint: ReunionRestSup[];
  getDiradjoint = false;
  ...
  ngOnInit(): void {
    ...
    // Data for Superviseur :
    this.getReunionSuperviseur();
    // Data for Inspecteur :
    this.getReunionInspecteur();
    this.getReunionResponsable();
    this.getReunionDirAdjoint();
  ...
  getReunionSuperviseur(): void {
    this.meswebservices.getReunionTeambyProfilId('inspecteur').toPromise()
      .then(
        resultat => {
          // Succes
          if (resultat.length > 0) {
            this.listeReunionTeamSuperviseur = resultat;
          }
          this.initableTeamSuperviseur();
        }
      )
  }


  initableTeamSuperviseur() {
    //
  ...
  /** */
  getReunionInspecteur(): void {
    this.meswebservices.getReunionTeambyProfilId('respreseau').toPromise()
      .then(
        resultat => {
          // Succes
          if (resultat.length > 0) {
            this.listeReunionTeamInspecteur = resultat;
          }
          this.initableTeamInspecteur();
        }
      )
  }


  initableTeamInspecteur() {
    //
    this.getInspecteur = true;
    setTimeout(function () {

      $('#tableInspecteur').DataTable({
        "pagingType": "full_numbers",
        "lengthMenu": [
          [10, 25, 50, -1],
          [10, 25, 50, "All"]
  ...
  /** */
  getReunionResponsable(): void {
    this.meswebservices.getAllManagersReunion('respreseau').toPromise()
      .then(
        resultat => {
          // Succes
          if (resultat.length > 0) {
            this.listeReunionTeamResponsable = resultat;
          }
          this.initableTeamResponsable();
        },
        (error) => {
          // 
        }
  ...
  /** */
  getReunionDirAdjoint(): void {
    this.meswebservices.getAllManagersReunion('dircomadj').toPromise()
      .then(
        resultat => {
          // Succes
          if (resultat.length > 0) {
            this.listeReunionTeamDiradjoint = resultat;
          }
          this.initableTeamDiradjoint();
        },
        (error) => {
          // 
        }
  ...
  initableTeamDiradjoint() {
    //
    this.getDiradjoint = true;
    setTimeout(function () {
