...
export class AgendaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getSuperviseurAgenda();
    this.getbackallrdv();
  }


  getTraderAgenda(): void {
    this.meswebservices.geTraderagendabyinsp().toPromise().then(
      resultat => {
  ...
  processSuperviseurDate() : void {    
    ...
    $calendar.fullCalendar({
      ...
      // color classes: [ event-blue | event-azure | event-green | event-orange | event-red ]
      events : this.evenementsSuperv      
    });
  }


  getSuperviseurAgenda(): void {
    this.meswebservices.getSupervAgendabyinsp().toPromise().then(
      resultat => {
        if (resultat.length > 0) {
          //  
          for(var i=0; i<resultat.length; i++){
            ...
            var events = {
              title: resultat[i].commercial,
              start : new Date(y, m, d, heure, minute),
              className: this.generatSuperviseurColor()
            }            
  ...
  // Generate color :
  generatSuperviseurColor() : string{
