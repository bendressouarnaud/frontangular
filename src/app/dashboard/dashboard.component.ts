...
import { MeswebservService } from '../messervices/meswebserv.service';
...
export class DashboardComponent implements OnInit, AfterViewInit {
    ...
    .then(
        resultat =>{
            ...
            if(resultat.length > 0){
              ...
              setTimeout(function(){ 
                $('#datatables, #datatablesrdv, #datatablesrapport').DataTable({
  ...
  /* get RDV list :*/
  getRdvlist(): void {
    this.meswebservices.getrdvliste().toPromise()
    .then(
      resultat => {
        if(resultat.length > 0){
          
          this.listeRdv = resultat;
          this.getListeRdv = true;
  ...
  /* get RAPPORT list :*/
  getRapportlist(): void {
    this.meswebservices.getrapportliste().toPromise()
    .then(
      resultat => {
        if(resultat.length > 0){
