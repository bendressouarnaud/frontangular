...
export class RdvComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // RDV from commercials' TEAM :
    this.getFullComRdvManager();
    this.getFullSupRdvManager();
    this.getFullInsRdvManager();
  ...
  // Get RDV list :
  getFullComRdvManager(): void {
    this.meswebservices.getFullComRdvManager().toPromise()
      .then(
        resultat => {
          // Succes
          if (resultat.length > 0) {
  ...
  // Get RDV list Superviseur :
  getFullSupRdvManager(): void {
    this.meswebservices.getFullSupRdvManager().toPromise()
      .then(
        resultat => {
          // Succes
          if (resultat.length > 0) {
  ...
  // Get RDV list Superviseur :
  getFullInsRdvManager(): void {
    this.meswebservices.getFullInsRdvManager().toPromise()
      .then(
        resultat => {
          // Succes
          if (resultat.length > 0) {
            this.listeRdvIns = resultat;
          }
    ...
    this.apicall.enregistrerRapport(this.rapport).toPromise().then(
      resultat => {
        if (resultat.element == "ok") {
          ...
          window.location.href = "#/responsable/rapport";
        }
      }
    )
  }
